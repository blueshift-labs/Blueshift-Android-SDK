...
public final class GCMRegistrar {
    ...
    /**
     * {@link com.blueshift.gcm.GCMBroadcastReceiver} instance used to handle the retry intent.
     *
     * <p>
     * This instance cannot be the same as the one defined in the manifest
     * because it needs a different permission.
     */
    private static com.blueshift.gcm.GCMBroadcastReceiver sRetryReceiver;
     ...
     *      {@value com.blueshift.gcm.GCMConstants#PERMISSION_GCM_INTENTS} permission.
     *    <li>The {@link android.content.BroadcastReceiver}(s) handles the 3 GCM intents
     *      ({@value com.blueshift.gcm.GCMConstants#INTENT_FROM_GCM_MESSAGE},
     *      {@value com.blueshift.gcm.GCMConstants#INTENT_FROM_GCM_REGISTRATION_CALLBACK},
     *      and {@value com.blueshift.gcm.GCMConstants#INTENT_FROM_GCM_LIBRARY_RETRY}).
     * </ol>
    ...
    public static boolean checkManifest(Context context) {
        ...
        for (ActivityInfo receiver : receivers) {
            if (com.blueshift.gcm.GCMConstants.PERMISSION_GCM_INTENTS.equals(
                    receiver.permission)) {
                allowedReceivers.add(receiver.name);
            }
        }
        if (allowedReceivers.isEmpty()) {
            Log.e(TAG,"No receiver allowed to receive " + com.blueshift.gcm.GCMConstants.PERMISSION_GCM_INTENTS);
            return false;
        }
        checkReceiver(context, allowedReceivers,
                com.blueshift.gcm.GCMConstants.INTENT_FROM_GCM_REGISTRATION_CALLBACK);
        checkReceiver(context, allowedReceivers,
                com.blueshift.gcm.GCMConstants.INTENT_FROM_GCM_MESSAGE);
        ...
        for (ResolveInfo receiver : receivers) {
            ...
            if (!allowedReceivers.contains(name)) {
                throw new IllegalStateException("Receiver " + name +
                        ...
                        com.blueshift.gcm.GCMConstants.PERMISSION_GCM_INTENTS);
            }
        }
    }
     ...
     * {@link com.blueshift.gcm.GCMConstants#INTENT_FROM_GCM_REGISTRATION_CALLBACK} intent with
     * either a {@link com.blueshift.gcm.GCMConstants#EXTRA_REGISTRATION_ID} or
     * {@link com.blueshift.gcm.GCMConstants#EXTRA_ERROR}.
     *
     * @param context application context.
     ...
     * <p>
     * The result will be returned as an
     * {@link com.blueshift.gcm.GCMConstants#INTENT_FROM_GCM_REGISTRATION_CALLBACK} intent with an
     * {@link com.blueshift.gcm.GCMConstants#EXTRA_UNREGISTERED} extra.
     */
    public static void unregister(Context context) {
        com.blueshift.gcm.GCMRegistrar.resetBackoff(context);
    ...
    /**
     * Lazy initializes the {@link com.blueshift.gcm.GCMBroadcastReceiver} instance.
     */
    static synchronized void setRetryBroadcastReceiver(Context context) {
        if (sRetryReceiver == null) {
            if (sRetryReceiverClassName == null) {
                ...
                sRetryReceiver = new com.blueshift.gcm.GCMBroadcastReceiver();
            } else {
                Class<?> clazz;
                try {
                    clazz = Class.forName(sRetryReceiverClassName);
                    sRetryReceiver = (com.blueshift.gcm.GCMBroadcastReceiver) clazz.newInstance();
                } catch (Exception e) {
                    Log.e(TAG, "Could not create instance of " +
                            sRetryReceiverClassName + ". Using " +
                            com.blueshift.gcm.GCMBroadcastReceiver.class.getName() +
                            " directly.");
