...
if [ -z "$1" ]; then
  ...
  printf "Usage: ./release-compose.sh <version>\n"
  exit 1
fi

VERSION=$1
BRANCH="main" # pick the current branch
BUILD_GRADLE="android-sdk-compose/build.gradle"
AAR_DEST="dist/"
...
M2_REPO_BASE="$HOME/.m2/repository/com/blueshift/android-sdk-compose"
M2_REPO_VERSION_DIR="$M2_REPO_BASE/$VERSION"
TAG_NAME="compose-v${VERSION}"

printf "\n🚀 Releasing android-sdk-compose version: %s\n" "$VERSION"
printf "🔍 Current branch: %s\n" "$BRANCH"

# --- Step 1: Update PUBLISH_VERSION in build.gradle ---
printf "\n📌 Step 1: Updating PUBLISH_VERSION in android-sdk-compose build.gradle...\n"
...
printf "\n📌 Step 2: Building android-sdk-compose AAR and publishing locally...\n"
./gradlew :android-sdk-compose:assembleRelease :android-sdk-compose:publishToMavenLocal
...
rm -f "$AAR_DEST"/android-sdk-compose-*.aar
...
AAR_SOURCE=$(find "$M2_REPO_VERSION_DIR" -maxdepth 1 -name "android-sdk-compose-$VERSION.aar" -print -quit)
...
if [ -f "$AAR_SOURCE" ]; then
  ...
  printf "🔍 Searched for pattern: android-sdk-compose-%s.aar\n" "$VERSION"
...
git commit -m "Published android-sdk-compose ${TAG_NAME} via Maven Central" || echo "ℹ️ No changes to commit"
...
printf "\n📌 Step 7: Publishing android-sdk-compose to Maven Central...\n"
...
./gradlew :android-sdk-compose:publishToMavenCentral
printf "✅ android-sdk-compose publish task submitted successfully!\n"
...
printf "🎉 android-sdk-compose release script completed successfully! 🚀\n"